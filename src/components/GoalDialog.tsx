...
export function GoalDialog({ onClose, onConfirmed }: { onClose: () => void; onConfirmed: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="modal goal-modal" role="dialog" aria-modal="true" aria-labelledby="goal-title">
    <header><h2 id="goal-title">设置营养目标</h2><button className="icon-button" onClick={onClose} aria-label="关闭"><Icon name="close" /></button></header>
    {!proposal ? <form className="form-stack" onSubmit={(event) => { event.preventDefault(); void propose(); }}>
      ...
      <fieldset className="safety-check"><legend>使用条件</legend><label><input type="checkbox" required checked={profile.generalHealthEligible} onChange={(e) => update('generalHealthEligible', e.target.checked)} />我已满 18 岁且未满 65 岁，且不属于妊娠或哺乳期，也没有饮食障碍、慢性病、持续用药或医生规定的特殊饮食。</label></fieldset>
      <p className="goal-reference-note">目标建议仅作参考，不替代医生或营养师的意见。</p>
